...
/*             if(gameMode === "random"){
            ...
            for(let i = itensAsked.length; i < numberIngredients; i++){
                ...
                let sorteado = parseInt(Math.random() * (number - 0) + 0);
                while(i > 0 && copyArray[sorteado] === itensAsked[i-1]){
                    sorteado = parseInt(Math.random() * (number - 0) + 0);
                }
                itensAsked.push(copyArray[sorteado]);
            }
        };
